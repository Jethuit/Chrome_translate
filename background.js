...
const CACHE_PROMPT_VERSION = 'twitter-v2';
...
async function reinjectIntoExistingTabs(reason) {
  ...
  const isFreshInstall = reason === 'install';
  ...
  await Promise.all(tabs.map(async (tab) => {
    ...
    try {
      ...
      // On update, the manifest-declared CSS is already present in the tab from
      // the previous load; injecting again would duplicate the stylesheet.
      // Only inject on fresh install, where the tab has never seen our CSS.
      if (isFreshInstall) {
...
function buildTranslationRequestBody(text, targetLang, settings, { stream = false } = {}) {
  const body = {
    ...
    messages: [
      {
        role: 'system',
        content: `You are a professional ${targetLang} native translator specialized in Twitter content who needs to fluently translate text into ${targetLang}.
...
7. Preserve the concise nature and informal tone typical of tweets`
      },
      {
        role: 'user',
        content: text
