(() => {
  ...
  if (typeof previousCleanup === 'function') {
    try {
      ...
      console.warn('LLM Web Translator: 清理旧实例失败', error);
    }
  }

  const TWEET_TEXT_SELECTOR = '[data-testid="tweetText"]';
  const DISCORD_MESSAGE_TEXT_SELECTOR = '[id^="message-content-"]';
  const DISCORD_MESSAGE_ITEM_SELECTOR = '[id^="chat-messages-"]';
  const DISCORD_REPLY_PREVIEW_SELECTOR = [
    '[id^="message-reply-context-"]',
    '[class*="repliedMessage"]',
    '[class*="repliedTextPreview"]',
    '[class*="replyContent"]'
  ].join(', ');
  const TRANSLATABLE_SELECTOR = `${TWEET_TEXT_SELECTOR}, ${DISCORD_MESSAGE_TEXT_SELECTOR}`;
  const EXTENSION_UI_SELECTOR = '.xtt-block, .xtt-loading, .xtt-error';
  const SCAN_ROOT_SELECTOR = 'main[role="main"], [role="dialog"], [aria-modal="true"], [data-list-id="chat-messages"], [role="log"]';
  const MIN_TEXT_LENGTH = 4;
  const PAGE_CACHE_MAX_SIZE = 300;
  const MAX_CONSECUTIVE_ERRORS = 3;
  const BELOW_VIEWPORT_SCAN_MARGIN = 300;
  ...
  const settingsReady = loadSettings().catch((error) => {
    console.error('LLM Web Translator: 加载设置失败', error);
  });
  ...
  function isTwitterTextElement(el) {
    return el.matches(TWEET_TEXT_SELECTOR);
  }

  function isDiscordMessageTextElement(el) {
    return el.matches(DISCORD_MESSAGE_TEXT_SELECTOR);
  }

  function isDiscordReplyPreviewElement(el) {
    if (!isDiscordMessageTextElement(el)) return false;
    if (el.closest(DISCORD_REPLY_PREVIEW_SELECTOR)) return true;

    const messageItem = el.closest(DISCORD_MESSAGE_ITEM_SELECTOR);
    if (!messageItem?.id || !el.id) return false;

    const messageId = el.id.replace(/^message-content-/, '');
    return Boolean(messageId) && !messageItem.id.endsWith(`-${messageId}`);
  }

  function shouldSkipTranslatableElement(el) {
    return isDiscordReplyPreviewElement(el);
  }
  ...
  function isInAutoTranslateRange(el) {
    const rect = el.getBoundingClientRect();
    return rect.bottom >= 0 && rect.top <= window.innerHeight + BELOW_VIEWPORT_SCAN_MARGIN;
  }

  function captureViewportAnchor() {
    const x = Math.max(0, Math.min(window.innerWidth - 1, window.innerWidth / 2));
    const yPoints = [0.35, 0.5, 0.65].map((ratio) => (
      Math.max(0, Math.min(window.innerHeight - 1, window.innerHeight * ratio))
    ));

    for (const y of yPoints) {
      const pointEl = document.elementFromPoint(x, y);
      if (!(pointEl instanceof Element)) continue;

      const anchorEl = pointEl.closest(
        `${DISCORD_MESSAGE_ITEM_SELECTOR}, article[data-testid="tweet"], main[role="main"], [role="dialog"], [aria-modal="true"]`
      ) || pointEl;
      if (isExtensionUiElement(anchorEl)) continue;
  ...
  function restoreViewportAnchor(anchor) {
    ...
    requestAnimationFrame(() => {
      ...
      if (lastUserScrollIntentAt > anchor.capturedAt) return;
  ...
  function preserveViewportAnchor(mutator) {
  ...
  function createTemporaryError(referenceEl, message) {
    ...
    setTimeout(() => {
      preserveViewportAnchor(() => errBlock.remove());
    }, 6000);
    return errBlock;
  }
  ...
  async function expandTweetTextIfNeeded(el) {
    ...
    const expansionAnchor = captureViewportAnchor();
    ...
    restoreViewportAnchor(expansionAnchor);
  ...
  function removeExtensionUiAfter(el) {
    ...
    if (next && next.matches(EXTENSION_UI_SELECTOR)) {
      preserveViewportAnchor(() => next.remove());
    }
  }
  ...
  function tryRenderCachedTranslation(el) {
    if (shouldSkipTranslatableElement(el)) return false;
  ...
  async function translateElement(el) {
    ...
    function flushStreamText() {
      if (!streamBlock?.isConnected || !pendingStreamText) return;

      const nextText = pendingStreamText;
      pendingStreamText = '';
      preserveViewportAnchor(() => {
        if (streamBlock?.isConnected) {
          streamBlock.textContent = nextText;
        }
      });
    }

    function scheduleStreamText(partialText) {
    ...
    try {
      ...
      const expandResult = isTwitterTextElement(activeEl)
        ? await expandTweetTextIfNeeded(activeEl)
        : { element: activeEl, blocked: false };
      ...
      const translated = await translateTextStream(text, settings.targetLang, (partialText) => {
        scheduleStreamText(partialText);
      });
  ...
  function findTranslatableText(target) {
    if (!(target instanceof Element)) return null;
    if (isExtensionUiElement(target)) return null;

    const directTextEl = target.closest(TRANSLATABLE_SELECTOR);
    if (directTextEl && isHoverTarget(directTextEl)) return directTextEl;
    ...
    const discordMessage = target.closest(DISCORD_MESSAGE_ITEM_SELECTOR);
    if (discordMessage) {
      const textEl = discordMessage.querySelector(DISCORD_MESSAGE_TEXT_SELECTOR);
      if (textEl && isHoverTarget(textEl)) return textEl;
    }
  ...
  function handleMouseMove(e) {
    ...
    const target = findTranslatableText(e.target);
  ...
  function cleanupInstance() {
    ...
    document.removeEventListener('wheel', noteUserScrollIntent);
    document.removeEventListener('touchstart', noteUserScrollIntent);
  ...
  async function init() {
    ...
    document.addEventListener('wheel', noteUserScrollIntent, { passive: true });
    document.addEventListener('touchstart', noteUserScrollIntent, { passive: true });
